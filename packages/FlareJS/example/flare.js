...
function N(o) {
  ...
  if (e != null && e.hasAttribute('href')) {
    ...
    if ((o.preventDefault(), n != null && n.startsWith('#'))) return (E(n), { type: 'scrolled' });
...
class T {
  constructor(e = {}) {
    ((this.enabled = !0),
      ...
      window != null && window.history
        ...
        : (console.warn('flare router not supported in this browser or environment'), (this.enabled = !1)));
  }
  go(e) {
  ...
  get allLinks() {
    return Array.from(document.links).filter(
      (e) =>
        e.href.includes(document.location.origin) && !e.href.includes('#') && e.href !== (document.location.href || `${document.location.href}/`) && !this.prefetched.has(e.href),
    );
  }
  log(...e) {
      ...
      if ((this.log('\u26A1', e), ['popstate', 'link', 'go'].includes(e) && t !== n)) {
        ...
        const i = await (
            await fetch(t, { headers: { 'X-Flare': '1' } })
              ...
              .then((a) => new Response(a, { headers: { 'Content-Type': 'text/html' } }))
          ).text(),
          c = L(i);
        (x(c),
